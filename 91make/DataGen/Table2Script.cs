...
{
	private static string outPath = Application.dataPath + "/Samples/ILRuntime/1.6.5/Demo/HotFix_Project~/Scripts/ConfigScript"; //输出路径

	[MenuItem("91make.top/Clear Scripts",false,0)]
	...
	[MenuItem("91make.top/CSV To Script", false, 11)]
	...
	[MenuItem("91make.top/Excel To Script", false, 12)]
...
public partial class {CLASS_NAME}
{
    ...
    public {ROW_NAME} FindById(int id)
    {
       return list.Find((c)=> c.id == id).As<{ROW_NAME}>();
    }
	...
	private static string ProcessFieldValueByType(string fieldValue, string fieldType)
	{
		switch (fieldType) // Type是小粒度的字段提示
		{
			...
			case "float":
				return $"{fieldValue}f";
			case "LFloat":
				return $"new LFloat(true,(long)({fieldValue}f * LFloat.Precision))";
			case "Vector3": // 1234, 1234.56
				{
					return Regex.Replace(fieldValue, @"([+|-]?\d+\.?\d*),\s*([+|-]?\d+\.?\d*),\s*([+|-]?\d+\.?\d*)", @"new Vector3( $1f, $2f, $3f)");
				}
			case "LVector3": // 1234, 1234.56
				{
					return Regex.Replace(fieldValue, @"([+|-]?\d+\.?\d*),\s*([+|-]?\d+\.?\d*),\s*([+|-]?\d+\.?\d*)", @"new LVector3(true,(long)($1f*LFloat.Precision), (long)($2f*LFloat.Precision),(long)($3f*LFloat.Precision))");
				}
		}
